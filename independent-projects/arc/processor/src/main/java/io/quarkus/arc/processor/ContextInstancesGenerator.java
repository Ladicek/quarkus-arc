...
public class ContextInstancesGenerator extends AbstractGenerator {
    ...
    private void createContextInstances(Gizmo gizmo, DotName scope) {
        ...
        gizmo.class_(generatedName, cc -> {
            ...
            Map<String, BeanFields> idToFields = new HashMap<>();
            int fieldIndex = 0;
            for (BeanInfo bean : beans) {
                ...
                idToFields.put(bean.getIdentifier(), new BeanFields(handleField, lockField, lockUpdaterField));
            }

            cc.defaultConstructor();

            Map<String, MethodDesc> lazyLocks = generateLazyLocks(cc, idToFields);
            generateComputeIfAbsent(cc, beans, idToFields, lazyLocks);
            generateGetIfPresent(cc, beans, idToFields);
            generateGetAllPresent(cc, idToFields);
            List<MethodDesc> remove = generateRemove(cc, beans, idToFields, lazyLocks);
            generateRemoveEach(cc, remove);
        });
    }

    private Map<String, MethodDesc> generateLazyLocks(ClassCreator cc, Map<String, BeanFields> idToFields) {
        ...
        Map<String, MethodDesc> result = new HashMap<>(idToFields.size());
        for (Map.Entry<String, BeanFields> namedFields : idToFields.entrySet()) {
            ...
            MethodDesc desc = cc.method("lazy" + fields.lock().name(), mc -> {
                ...
                mc.body(b0 -> {
                    ...
                    // TODO see if we don't need to read the field here again, we shouldn't but let's see properly!
                    b0.return_(lock);
                });
            });
    ...
    private void generateComputeIfAbsent(ClassCreator cc, List<BeanInfo> beans, Map<String, BeanFields> idToFields,
            Map<String, MethodDesc> lazyLocks) {
        Map<BeanInfo, MethodDesc> computeMethodsByBean = new HashMap<>();
        for (BeanInfo bean : beans) {
            BeanFields fields = idToFields.get(bean.getIdentifier());
            ...
            MethodDesc desc = cc.method("c" + fields.instance().name(), mc -> {
                ...
                mc.body(b0 -> {
                    ...
                    LocalVar lock = b0.localVar("lock", b0.invokeVirtual(lazyLocks.get(bean.getIdentifier()), cc.this_()));
                    b0.invokeInterface(MethodDescs.LOCK_LOCK, lock);
                    b0.try_(tc -> {
                        tc.body(b1 -> {
                        ...
                        tc.finally_(b1 -> {
                            b1.invokeInterface(MethodDescs.LOCK_UNLOCK, lock);
                        });
                    });
                });
            });
            computeMethodsByBean.put(bean, desc);
        }

        cc.method("computeIfAbsent", mc -> {
            mc.returning(ContextInstanceHandle.class);
            ParamVar beanId = mc.parameter("beanId", String.class);
            ParamVar supplier = mc.parameter("supplier", Supplier.class);
            mc.body(b0 -> {
                b0.return_(b0.switch_(ContextInstanceHandle.class, beanId, sc -> {
                    for (BeanInfo bean : beans) {
                        sc.caseOf(bean.getIdentifier(), b1 -> {
                            b1.return_(b1.invokeVirtual(computeMethodsByBean.get(bean), cc.this_(), supplier));
                        });
                    }
                    sc.default_(b1 -> {
    ...
    private void generateGetIfPresent(ClassCreator cc, List<BeanInfo> beans, Map<String, BeanFields> idToFields) {
        cc.method("getIfPresent", mc -> {
            ...
            mc.body(b0 -> {
                b0.return_(b0.switch_(ContextInstanceHandle.class, beanId, sc -> {
                    for (BeanInfo bean : beans) {
                        sc.caseOf(bean.getIdentifier(), b1 -> {
                            b1.yield(cc.this_().field(idToFields.get(bean.getIdentifier()).instance()));
                        });
                    }
                    sc.default_(b1 -> {
                        b1.throw_(IllegalArgumentException.class, "Unknown bean identifier");
    ...
    private void generateGetAllPresent(ClassCreator cc, Map<String, BeanFields> idToFields) {
        cc.method("getAllPresent", mc -> {
            mc.returning(Set.class);
            mc.body(b0 -> {
                // ContextInstanceHandle<?> h1 = this.h1;
                // Set<ContextInstanceHandle<?>> result = new HashSet<>();
                // if (h1 != null) {
                //    result.add(h1);
                // }
                // return result;
                List<LocalVar> handles = new ArrayList<>(idToFields.size());
                for (BeanFields fields : idToFields.values()) {
                    handles.add(b0.localVar(fields.instance.name(), cc.this_().field(fields.instance)));
                }
    ...
    private List<MethodDesc> generateRemove(ClassCreator cc, List<BeanInfo> beans, Map<String, BeanFields> idToFields,
            Map<String, MethodDesc> lazyLocks) {

        // There is a separate remove method for every instance handle field
        // To eliminate large stack map table in the bytecode
        List<MethodDesc> removeMethods = new ArrayList<>(beans.size());
        Map<BeanInfo, MethodDesc> removeMethodsByBean = new HashMap<>();
        for (BeanInfo bean : beans) {
            BeanFields fields = idToFields.get(bean.getIdentifier());
            ...
            removeMethodsByBean.put(bean, desc);
        }
        ...
        cc.method("remove", mc -> {
            ...
            mc.body(b0 -> {
                b0.return_(b0.switch_(ContextInstanceHandle.class, beanId, sc -> {
                    for (BeanInfo bean : beans) {
                        sc.caseOf(bean.getIdentifier(), b1 -> {
                            b1.return_(b1.invokeVirtual(removeMethodsByBean.get(bean), cc.this_()));
                        });
                    }
                    sc.default_(b1 -> {
                        b1.throw_(IllegalArgumentException.class, "Unknown bean identifier");
