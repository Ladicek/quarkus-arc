...
public class ContextInstancesGenerator extends AbstractGenerator {
    ...
    private void createContextInstances(Gizmo gizmo, DotName scope) {
        ...
        gizmo.class_(generatedName, cc -> {
            ...
            Map<BeanInfo, BeanFields> beanFields = new HashMap<>();
            int fieldIndex = 0;
            for (BeanInfo bean : beans) {
                ...
                beanFields.put(bean, new BeanFields(handleField, lockField, lockUpdaterField));
            }

            cc.defaultConstructor();

            Map<BeanInfo, MethodDesc> lazyLocks = generateLazyLocks(cc, beanFields);
            generateComputeIfAbsent(cc, beans, beanFields, lazyLocks);
            generateGetIfPresent(cc, beans, beanFields);
            generateGetAllPresent(cc, beanFields);
            List<MethodDesc> remove = generateRemove(cc, beans, beanFields, lazyLocks);
            generateRemoveEach(cc, remove);
        });
    }

    private Map<BeanInfo, MethodDesc> generateLazyLocks(ClassCreator cc, Map<BeanInfo, BeanFields> beanFields) {
        ...
        Map<BeanInfo, MethodDesc> result = new HashMap<>(beanFields.size());
        for (Map.Entry<BeanInfo, BeanFields> namedFields : beanFields.entrySet()) {
            BeanInfo bean = namedFields.getKey();
            ...
            result.put(bean, desc);
        }
        return result;
    }

    private void generateComputeIfAbsent(ClassCreator cc, List<BeanInfo> beans, Map<BeanInfo, BeanFields> beanFields,
            Map<BeanInfo, MethodDesc> lazyLocks) {
        Map<BeanInfo, MethodDesc> computeMethodsByBean = new HashMap<>();
        for (BeanInfo bean : beans) {
            BeanFields fields = beanFields.get(bean);
            ...
            MethodDesc desc = cc.method("c" + fields.instance().name(), mc -> {
                ...
                mc.body(b0 -> {
                    ...
                    LocalVar lock = b0.localVar("lock", b0.invokeVirtual(lazyLocks.get(bean), cc.this_()));
    ...
    private void generateGetIfPresent(ClassCreator cc, List<BeanInfo> beans, Map<BeanInfo, BeanFields> beanFields) {
        cc.method("getIfPresent", mc -> {
            ...
            mc.body(b0 -> {
                b0.return_(b0.switch_(ContextInstanceHandle.class, beanId, sc -> {
                    for (BeanInfo bean : beans) {
                        sc.caseOf(bean.getIdentifier(), b1 -> {
                            b1.yield(cc.this_().field(beanFields.get(bean).instance()));
                        });
                    }
                    sc.default_(b1 -> {
                        b1.throw_(IllegalArgumentException.class, "Unknown bean identifier");
    ...
    private void generateGetAllPresent(ClassCreator cc, Map<BeanInfo, BeanFields> beanFields) {
    ...
    private List<MethodDesc> generateRemove(ClassCreator cc, List<BeanInfo> beans, Map<BeanInfo, BeanFields> beanFields,
            Map<BeanInfo, MethodDesc> lazyLocks) {
